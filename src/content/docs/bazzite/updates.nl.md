...
Desktop-images werken zichzelf op de achtergrond bij, dus in de praktijk draai je dit zelden met de hand. Meestal draai je het vanaf een TTY of via SSH. `ujust update` dekt de image, de Flatpaks en de distrobox-containers in één keer.
...
Hij staat ook in het bootmenu, en dat is wat je redt als het probleem je niet meer bij een terminal laat komen. Dit is het grootste praktische verschil met een rolling distributie.
...
`rpm-ostree upgrade` (en `bootc upgrade`, waar bootc gebruikt wordt) zet de volgende image klaar. Je bent niet bijgewerkt als het commando klaar is. Je bent bijgewerkt na de volgende herstart. Zo werkt `rpm-ostree`, en het is het meest verwarrende aan het bijwerken van een atomic systeem.
...
Topgrade is één commandline-binary, dus gebruik Homebrew. Geen layering, geen herstart, en hij werkt zichzelf bij samen met alles wat hij beheert:
...
Sla firmware over. `fwupdmgr` wil op de G16 herstarten naar de firmware-updater, wat zelden is wat je midden in een update wilt:
...
Minder dan op CachyOS. De image werkt zichzelf bij, `ujust update` dekt image plus Flatpaks plus distrobox al, en door het klaarzetten-maar-niet-toepassen is Topgrades uitvoer hier makkelijk verkeerd te lezen. Het verdient zijn plek als je op Homebrew en zelfgemaakte containers leunt. Doe je dat niet, dan is `ujust update` genoeg.
