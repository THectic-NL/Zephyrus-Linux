...
Desktop images update themselves in the background, so in practice you rarely run this by hand — mostly from a TTY or over SSH. `ujust update` covers the image, the Flatpaks and the distrobox containers in one go.
...
It's in the boot menu too, which is what saves you when the problem stops you reaching a terminal. This is the single biggest practical difference with a rolling distribution.
...
`rpm-ostree upgrade` (and `bootc upgrade`, where bootc is in use) **stages** the next image. You are not up to date when the command finishes — you are up to date after the next reboot. That's `rpm-ostree`'s behaviour, and it's the single most confusing thing about updating an atomic system.
...
Skip firmware — `fwupdmgr` on the G16 wants a reboot into the firmware updater, rarely what you want mid-update:
...
Less than on CachyOS, honestly. The image updates itself, `ujust update` already covers image plus Flatpaks plus distrobox, and the staged-not-applied behaviour makes Topgrade's output easier to misread here. It earns its place if you lean on Homebrew and self-made containers; if you don't, `ujust update` is enough.
