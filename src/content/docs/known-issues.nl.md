...
**Workaround:**
Schrijf rechtstreeks naar het sysfs-attribuut dat moet veranderen, buiten asusd's kapotte batch-write om — zie de firmware-attributentabel in de sectie "GPU mode switching" op de [asusctl-pagina]({{< relref "/docs/hardware/asusctl-rog-control" >}}) voor de exacte waarden en het pad. Nog niet bevestigd of dit ook daadwerkelijk werkende helderheid op Integrated oplevert, alleen dat het de betrouwbaardere manier is om de mode zelf te laten wisselen. De eigen upstream-suggestie, terugvallen op `supergfxctl`, botst direct met de eigen waarschuwing [hierboven]({{< relref "/docs/hardware/asusctl-rog-control" >}}) op deze pagina dat supergfxctl onbeheerd is en een beveiligingsrisico vormt — weeg die afweging zelf voordat je het als schone fix behandelt. De echte oplossing is een nieuwere asusctl-build; de moeite waard om te checken of er een `-git`/rolling package beschikbaar is die `main` sneller volgt dan het kanaal waar de huidige vandaan komt.
