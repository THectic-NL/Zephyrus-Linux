...
**Cross-platform kanttekening:** een vergelijkbaar klinkend probleem bestaat op Windows via [G-Helper op een 2023 Zephyrus G14 (GA402NI)](https://github.com/seerge/g-helper/issues/660) — ook daar wordt de helderheid onbestuurbaar rond een GPU-modewissel. Het exacte symptoom (vastzitten op lage helderheid bij wisselen ​*naar* dGPU/performance mode, in plaats van volledig onbestuurbaar in iGPU-only mode) en het hardwaremodel verschillen genoeg dat dit niet bevestigd is als hetzelfde mechanisme, en ook daar heeft geen maintainer de oorzaak vastgepind. Beschouw het als "dit soort MUX/EC-gebonden brightness-probleem is niet uniek voor Linux", niet als bewijs van een identieke bug.
