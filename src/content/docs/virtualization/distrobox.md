...
It is not a VM and not a sandbox. A distrobox container shares your home directory and runs as you. It is a way to get a *different distribution's packages*, not a way to isolate anything from yourself.
...
First entry takes a moment while it sets up your user inside the container and wires up the home directory. After that it is a shell.
...
An Arch container is mostly redundant here, since you are already on Arch. Reach for Ubuntu or Debian instead, which is where distrobox earns its place on this distribution.
...
Containers are meant to be disposable. If one gets into a state you can't explain, delete it and make a new one. That is cheaper than debugging it, and your home directory is untouched either way.
